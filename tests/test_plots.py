...
def test_local_contrast_1D_plot_draws_four_panels():
    ...
    pyspeckle.local_contrast_plot(speckle, np.ones(25))
    axes = plt.gcf().get_axes()
    assert len(axes) == 4
...
def test_local_contrast_1D_plot_aligns_the_two_traces():
    ...
    speckle = pyspeckle.create_exponential(512, 8)
    n = 25
    pyspeckle.local_contrast_plot(speckle, np.ones(n))
    axes = plt.gcf().get_axes()
...
def test_local_contrast_2D_plot_draws_four_panels():
    ...
    speckle = pyspeckle.create_exponential((64, 64), 2)
    pyspeckle.local_contrast_plot(speckle, np.ones((5, 5)))
    axes = plt.gcf().get_axes()
    assert len(axes) == 4
    assert "Overall Contrast" in axes[0].get_title()
    assert axes[2].get_title() == "Local speckle contrast"
...
def test_statistics_plot_rejects_a_volume():
    """A 3D pattern has no sensible 2x2 view; slice_plot is the one to use."""
    volume = pyspeckle.create_exponential((8, 8, 8), 2)
    with pytest.raises(ValueError):
        pyspeckle.statistics_plot(volume)


def test_local_contrast_plot_rejects_a_volume():
    """A 3D pattern has no sensible 2x2 view."""
    volume = pyspeckle.create_exponential((8, 8, 8), 2)
    with pytest.raises(ValueError):
        pyspeckle.local_contrast_plot(volume, np.ones((3, 3, 3)))


def test_plot_histogram_colors_are_unchanged():
    """statistics_plot uses gray bars, local_contrast_plot the default color."""
    speckle = pyspeckle.create_exponential(2048, 8)

    pyspeckle.statistics_plot(speckle)
    gray = plt.gcf().get_axes()[1].patches[0].get_facecolor()[:3]
    assert np.allclose(gray, 128 / 255)  # matplotlib "gray"

    plt.close("all")
    pyspeckle.local_contrast_plot(speckle, np.ones(25))
    default = plt.gcf().get_axes()[1].patches[0].get_facecolor()[:3]
    assert not np.allclose(default, 128 / 255)
